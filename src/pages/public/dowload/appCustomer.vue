<template>
	<div id="dowload-member">
		<div class="content">
			<div class="dow-btn-warp">
				<div class="dow-btn">
					<div class="dow" @click="onShowClick"><van-icon name='down'></van-icon>下载顾客客户端</div>
				</div>
			</div>
...
<script>
	export default {
		...
		mounted() {
			this.util.request({
				url: 'system/common/download/customer',
				method: 'POST',
			}).then((result) => {
...
<style>
	#dowload-member .content{
		...
		overflow-x: hidden;
		overflow-y: scroll;
		background: url(../../../../static/img/app-customer.png) no-repeat;
...
// WEBPACK FOOTER //
// src/pages/public/dowload/appCustomer.vue