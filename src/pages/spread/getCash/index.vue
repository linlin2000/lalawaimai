										...
										<router-link to="">
											<div class="item-inner van-hairline--bottom">
												<div class="item-title">
													...
													<div class="time">申请时间:{{item.addtime}}</div>
												</div>
  		...
  		methods: {
			onLoad() {
				...
				this.util.request({
					...
					data: {
						menufooter: 1,
						min: this.records.min,
						status : this.status
					}
				}).then((res) => {
					let result = res.data.message;
					this.records.data = [...this.records.data, ...result.message];
					if(!this.records.data.length) {
						this.records.empty = true;
					}
					this.records.loading = false;
					this.records.min = result.min;
					if(!result.min) {
...
// WEBPACK FOOTER //
// src/pages/spread/getCash/index.vue