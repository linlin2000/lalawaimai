<template>
	<div id="store-goods">
    <div class="popup-container" v-show="shengjiinfo.saomasj==1">
      <div class="tip1 closep">
         <div @click="newclose()" class="newclose">
        <img src="static/img/newclose.png"  />
        </div>
        <p class="tip_p" >恭喜您完成升级任务</p>
        <p class="tip_p">获得{{shengjiinfo.saomasjvalue}}成长值升级为{{shengjiinfo.saomasjname}}</p>
        <p class="tip_p tip_p2">福利升级</p>
        <div >
          <ul>
            <li class="tip_li" v-for="(item,index) in shengjiinfo.saomasjcoupon">
              <div class="tip_lit" style="display:flex;flex-wrap:wrap;margin: 0 auto;">
                <div class="tip_lit1" style="font-size:10px;flex:1;text-align:left;">
                  <b>￥</b><b style="font-size:22px;font-weight: 700;">{{item.discount}}</b>
                  <br />满{{item.condition}}可用
                </div>
                <!-- <div style="flex:1;font-size:12px;">黄金</div> -->

                <div style="font-size:10px;flex:3;text-align:right;">{{item.title}}
                  <br />{{item.starttime}}至{{item.endtime}}
                </div>
              </div>

              <div style="display: flex;text-align:left;font-size:10px;">
                <div style="flex:1">本次可用</div>
                <div style="flex:1" @click="touse()">去使用</div>
              </div>
            </li>
          </ul>
        </div>
    ...
    <div class="popup-container" v-show="showmeals==1">

      <div class="tip1 tip2">
        <!-- <h2><span @click="tests()">测试发送</span> <br/>选择就餐人数 <br/>得到信息是:<span v-for="(item,index) in test2">{{item}}</span></h2> -->
        <h2>选择就餐人数</h2>
        <div style="display:flex;flex-wrap: wrap;margin-top:12px;">
          <div :class="tempmeals == 1 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(1)">1</div>
          <div :class="tempmeals == 2 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(2)">2</div>
          <div :class="tempmeals == 3 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(3)">3</div>
          <div :class="tempmeals == 4 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(4)">4</div>
          <div :class="tempmeals == 5 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(5)">5</div>
          <div :class="tempmeals == 6 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(6)">6</div>
          <div :class="tempmeals == 7 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(7)">7</div>
          <div :class="tempmeals == 8 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(8)">8</div>
          <div :class="tempmeals == 9 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(9)">9</div>
          <div :class="tempmeals == 10 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(10)">10</div>
          <div :class="tempmeals == 11 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(11)">11</div>
          <div :class="tempmeals == 12 ? 'flexitem active1' : 'flexitem'"   @click="selectperson(12)">12</div>
          <!-- <div :class="flexitem" @click="selectperson(4)">4</div>
          <div :class="flexitem" @click="selectperson(5)">5</div>
          <div :class="flexitem" @click="selectperson(6)">6</div>
          <div :class="flexitem" @click="selectperson(7)">7</div>
          <div :class="flexitem" @click="selectperson(8)">8</div>
          <div :class="flexitem" @click="selectperson(9)">9</div>
          <div :class="flexitem" @click="selectperson(10)">10</div>
          <div :class="flexitem" @click="selectperson(11)">11</div>
          <div :class="flexitem" @click="selectperson(12)">12</div> -->
        </div>
        <div style="display:flex;margin-top: 20px;text-align:right">
          <div style="width:26.67%;height:30px;line-height:30px;font-size:14px;text-align:left">自定义</div>
          <div><input class="custp" v-model="tempmeals"></div>
        </div>
      </div>
      <div class="p_end" @click="sdc()">开始点餐</div>


    </div>
    <div class="tip1 righttip" v-show="bottom_tips.cha_sj==0 && bottom_tips.is_have_coupon==1" @click="showcoupon()">优惠券
    <br/>
      <div style="margin-top:8px;color:white;">{{bottom_tips.cha_sj_name}}</div>

    </div>
   <div class="popup-container" v-show="showcoupons==1">
      <div class="tip1 closep" >
        <div @click="newclose2()" class="newclose">
         <img  src="static/img/newclose.png"  />
         </div>
        <div >
          <ul>
             <li class="tip_li" v-for="(item,index) in bottom_tips.coupon">
               <div class="tip_lit" style="display:flex;flex-wrap:wrap;margin: 0 auto;">
                 <div class="tip_lit1" style="font-size:10px;flex:1;text-align:left;">
                   ...
                   <div v-if="item.discount ==''">
                    <b style="font-size:22px;font-weight: 700;">{{item.zhekou}}</b><b>折</b>
                   </div>
                   <br />满{{item.condition}}可用
                 </div>
               ...
               <div style="display: flex;text-align:left;font-size:10px;">
                 <div style="flex:1">本次可用</div>
                 <div style="flex:1" @click="touse('yhj')">去使用</div>
               </div>
             </li>
          </ul>
        </div>
    ...
    <store-header :store="store" :table="table"></store-header>
		<!-- 标签 -->
					...
					<van-list v-model="goodsLoading" :finished="goodsItem.loaded" @load="onGetGoods" :offset="50">
						...
						<div class="banner" v-if="0 && shopPage">
							<template v-for="(shopPageItem, shopPageKey) in shopPage">
								...
								<router-link class="banner-item" v-else-if="shopPageItem.goodsLength == 1" :to="util.getUrl({path: '/tangshi/pages/table/goodsDetail', query:{sid: store.id, id: shopPageItem.goods[0]}})">
									<img :src="shopPageItem.thumb">
								</router-link>
						...
						<template v-if="categorySelected.child">
							<div class="tag2-list" v-if="categorySelected.child.length > 0">
								<div class="tag2-item" v-for="(item, index) in categorySelected.child" :class="{selected: index == childSelectedIndex}" @click="onToggleCategory(item.id, index, 'child')" :key="index">
									{{item.title}}
						...
						<template v-if="template == 2">
							<div class="goods clearfix " v-if="goodsItem.data.length > 0" v-for="(item, index) in goodsItem.data" :key="item.id">
								<div class="goods-inner border-1px-b">
									<router-link class="image-box" :to="util.getUrl({path: '/tangshi/pages/table/goodsDetail', query: {id: item.id, sid: store.id, table_id: table_id, order_id: order_id}})">
										<img class="pic" v-lazy="{src: item.thumb, loading: lazyload_goods}">
										<div class="label" v-if="item.label">{{item.label}}</div>
									</router-link>
									<div class='shop-info clearfix'>
										<div class="name ellipsis custwidth">{{item.title}}</div>
										...
										<div class="price">
											...
											<div class='old-price' v-if="item.old_price"  style=" position: initial;">
						...
						<template v-else>
							<div class="goods-list clearfix" v-if="goodsItem.data && goodsItem.data.length > 0" >
								<div class="col-50 goods-item" v-for="(item, index) in goodsItem.data" :key="item.id">
									<div class="goods-info">
										<router-link class="avatar" :to="util.getUrl({path: '/tangshi/pages/table/goodsDetail', query: {id: item.id, sid: store.id, table_id: table_id, order_id: order_id}})">
				...
				<van-col span="19" :style="{height: vanColHeight +'px'}">
					<div ref="goodsWrapper" :style="{height: vanColHeight +'px'}" >
						<div>
							...
							<!-- 门店活动 -->
              <div class="banner"  v-if="huodong">
              	<template>
              		<div class="banner-item" v-if="huodong.wxapp_link"  @click="util.jsUrl(huodong.wxapp_link+'&table_id='+table_id)" >
              			<img :src="'https://123.91ydxk.com/attachment/'+huodong.thumb">
              		</div>
              	</template>
              </div>
              <!-- 门店海报 -->
							<div class="banner" ref="shopPageWrapper" v-if="shopPage && 0">
								<template v-for="(shopPageItem, shopPageKey) in shopPage">
									<div class="banner-item" v-if="shopPageItem.wxapp_link"  @click="util.jsUrl(shopPageItem.wxapp_link)" >
										<img :src="'attchment/'+shopPageItem.thumb">
									</div>
									<router-link class="banner-item" v-else-if="shopPageItem.goodsLength == 1" :to="util.getUrl({path: '/pages/goods/goodsDetail', query:{sid: store.id, id: shopPageItem.goods[0]}})">
										<img :src="shopPageItem.thumb">
									</router-link>
									<router-link class="banner-item" v-else-if="shopPageItem.goodsLength > 1" :to="util.getUrl({path: '/pages/store/shopPage', query:{sid: store.id, shopPageKey: shopPageKey}})">
										<img :src="shopPageItem.thumb">
							...
							<ul>
								<li v-for="(citem, cindex) in cateHasGoods" :key="cindex" ref="goodsList">
									...
									<!--张月芳 单列模板 堂食-->
									<template v-if="template == 2">
										<div class="goods clearfix" v-if="citem.goods.length > 0" v-for="(item, index) in citem.goods" :key="item.id">
                            ...
                            <div class='old-price' v-if="item.old_price">
                              <div class="price-icon">￥</div>{{item.old_price}}
														...
														<div class="now-price" style=" position: initial;">
															<span class="price-icon font-14">￥</span>{{item.price}}<span class="font-14 font-bold">{{item.unitname_cn}}</span>
														</div>
                          ...
                          <div class='discount' v-if="(item.svip_status != 1 || item.bargain_id > 0) && item.notvip==1">
                          	<template v-if="item.discount">
                          		<van-icon name='tag'></van-icon>
                          		会员价：{{item.fans_show_vip_price}}元
                          	</template>
                          </div>
                          <div class='discount' v-if="(item.svip_status != 1 || item.bargain_id > 0) && item.notvip==0">
                          	<template v-if="item.discount">
                          		<van-icon name='tag'></van-icon>
                          		会员价：{{item.price}}元
                          	</template>
									...
									<template v-else>
										<div class="goods-list clearfix" v-if="citem.goods && citem.goods.length > 0" >
											<div class="col-50 goods-item" v-for="(item, index) in citem.goods" :key="item.id">
												<div class="goods-info">
													<router-link class="avatar" :to="util.getUrl({path: '/tangshi/pages/table/goodsDetail', query: {id: item.id, sid: store.id, table_id: table_id, order_id: order_id}})">
														<img v-lazy="{src: item.thumb, loading: lazyload_goods}">
													</router-link>
                            ...
                            <div class="price">
                              <div v-if="item.bargain_id>0">
                                ¥<div>{{item.price}}</div>
                                <span class="font-bold" v-show="item.unitname">/{{item.unitname}}</span>
                                <span class="solid">原价:{{item.ts_price}}/{{item.unitname}}</span>
                              </div>
                              <div v-else>
                                ¥<div>{{item.ts_price}}</div>
                                <span class="font-bold" v-show="item.unitname">/{{item.unitname}}</span>
                              </div>
                            </div>


													<div v-if="item.pay_type == 1" class="price">¥
														<div>{{item.price}}</div><span class="font-bold">{{item.unitname_cn}}</span>
													</div>
														<div v-if="item.pay_type == 2" class="price">分
														<div>{{item.integral}}</div><span class="font-bold">{{item.unitname_cn}}</span>
													</div>
													<div class="old-price" v-if="0 && item.old_price">{{item.old_price}}</div>
													<div class="discount" v-if="(item.svip_status != 1 || item.bargain_id > 0) && item.discount > 0 && item.notvip==0">
														<div>
															<van-icon name="tag"></van-icon>
													   会员价：{{item.price}}元
													...
													<div class="discount" v-if="(item.svip_status != 1 || item.bargain_id > 0) && item.discount > 0 && item.notvip==1">
														<div>
															<van-icon name="tag"></van-icon>
													   会员价：{{item.fans_show_vip_price}}元
													  </div>
...
<van-popup v-model="ParentPopupSpecShow" class="options-popup" v-if="ParentPopupSpecShow">
    ...
    <div class="grouptip" v-show="bottom_tips.cha_sj==1">
      <div>
        还差<b style="color:mediumvioletred">{{bottom_tips.cha_sj_money}}</b>元 即可完成{{bottom_tips.cha_sj_name}}任务
        <span @click="cd()" style="margin-left:36px;">去凑单</span>
      </div>
    </div>

    <div class="grouptip" v-show="bottom_tips.cha_sj==0 && bottom_tips.is_have_coupon==1">
      <div>
        还差<b style="color:mediumvioletred">{{bottom_tips.cha_sj_money}}</b>元 即可使用{{bottom_tips.cha_sj_name}}
        <span @click="cd('mj')" style="margin-left:36px;">去凑单</span>
      </div>
    </div>
	...
	export default {
      ...
      differce:function(value){
        console.log("differce");
         this.bottom_tips = value;

        //发送node服务端
         //  let that = this ;
         //  let ws = this.comjs.linknode();
         ...
         // console.log('结果2==',that.testval);
      },
      newclose:function(){
        this.shengjiinfo.saomasj = 0;
      },
      newclose2:function(){
      ...
      sdc:function(){
         ...
         if(!this.tempmeals || isNaN(this.tempmeals)==true || this.tempmeals<=0){
            alert("请正确填写来客人数");
            return false;
         }
        this.showmeals = 0;
      ...
      cd:function(value){
        this.bottom_tips.cha_sj = 0;
        if(value=='mj'){
          this.bottom_tips.is_have_coupon = 0;
        }

      },
      touse:function(value){
        if(value=='yhj'){
          this.showcoupons = 0;
        }else{
          this.shengjiinfo.saomasj=0;
      ...
      selectperson:function(value){
        this.tempmeals = value;

        //console.log(this.tempmeals,'人数');
      },
      showcoupon(){
        this.showcoupons = 1;
      },
     ...
     //单选择变多选
			onParentToggleOption: function (optionId, optionValueID, type,is_muti) {
        ...
        //多选
        //goodsActive['activeOptions']['attrsSelected'][0] = '0,1';
        ...
        var goods_id = goods.id || goods.goods_id;
			...
			onLoad() {
				var table = this.util.getStorage("table_id");
				if(!table && this.table_id){
					this.util.setStorage("table_id", this.table_id);
				}
				this.util.request({
					url: 'wmall/store/table/index_vue',
					data: {
						sid: this.sid,
						cid: this.cid,
						table_id: this.table_id ? this.table_id : table,
          ...
          this.pd_cart = result.pdcart;

          // var temp_cart = result.cart.message.cart.data;
          // if(temp_cart){
          //   //提示差多少元
          //   this.bottom_tips.cha_sj = 1;
          // }
          ...
          if(result.store.data && result.store.data.huodong) {
          	this.huodong = result.store.data.huodong;
          }

					this.table = result.table;
					this.table.jiacai_id = this.order_id;
           ...
           this.shengjiinfo = result.shengjiinfo;//升级info
          ...
          //如果手机必填项提示
          if(this.store.fillmobile==1 && result.otherinfo.havemobile==0 ){
            this.$dialog.alert({
               title: "温馨提示",
               message: "点餐前须填写手机号",
            ...
            .then(() => {
                window.location = result.otherinfo.bindmobile;
            })
          }else{
            if(this.shengjiinfo.saomasj==0 && !this.order_id){
              this.showmeals = 1;
            }
        ...
        //失效时间戳
        //console.log(this.query.time_stamp);
        this.time_stamp = this.query.time_stamp;
...
<style>
  ...
  .active1{
    ...
    border: solid 1px rgb(231,56,34);
  }


  .flexitem:nth-child(4),.flexitem:nth-child(8),.flexitem:nth-child(12){
      margin-right: -2px;
   }
  .custp{
    border-color: rgb(187, 187, 187);
  ...
  .p_end{
    margin-left:5%;
    margin-top:-15px;
    background-color: rgb(231, 56, 34);
    text-align:center;
    height:36px;
    line-height:36px;
  ...
  #store-goods .righttip{
    ...
    transform-origin: 0 50;
      animation: moveImg 1s ease infinite 0s alternate;
  ...
  #store-goods .goods-container .goods .shop-info .custwidth{
    max-width:195px;
  }
  ...
  #store-goods .goods-list .goods-item .price .solid{
  	display: inline-block;
  	font-size: 10px;
  	font-weight: bold;
  	line-height:10px;
