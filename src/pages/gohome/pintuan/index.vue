	...
	<div id="pintuan-index">
		...
		<div class="content">
			<van-pull-refresh v-model="isRefresh" @refresh="onPullDownRefresh()">
				<div class="pintuan-navs margin-10-b">
					<van-swipe indicator-color="#ff2d4b">
						<van-swipe-item v-for="(tempitem, index) in navs" :key="tempitem.id">
							<div class="pintuan-navs-container col-5 circle">
								<div class="pintuan-nav-col" v-for="(item, index) in tempitem" :key="item.id" @click="util.jsUrl(item.link)">
									<div class="pintuan-nav-icon">
										<img :src="item.thumb">
									</div>
									<div class="pintuan-nav-text">{{item.title}}</div>
								</div>
							</div>
						</van-swipe-item>
					</van-swipe>
...
// WEBPACK FOOTER //
// src/pages/gohome/pintuan/index.vue