		...
		methods: {
			...
			onLoad() {
				this.util.request({
					...
					if(result.errno) {
						if(result.errno == -1000) {
							this.$router.replace(this.util.getUrl({path: '/gohome/pages/haodian/application'}));
							return;
						}
						this.message = {
							type: 'error',
...
<style scoped>
	#haodian-settle .content{
		padding-top: 10px;
	}
	#haodian-settle .content .submit{
		margin-top: 20px;
		padding: 0 15px;
	}
	#haodian-settle .content .email-img{
		height: 32px;
		width: 90px;
		position: absolute;
		top: 60px;
	...
	#haodian-settle .content .email-img img{
		border: none;
		width: 100%;
		height: 100%;
	}
	#haodian-settle .content .list-password{
		z-index: 10000;
		position: absolute;
		top: 108px;
		right: 15px;
...
// WEBPACK FOOTER //
// src/pages/gohome/haodian/settle.vue